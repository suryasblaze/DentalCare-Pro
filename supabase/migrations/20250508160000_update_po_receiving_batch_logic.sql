-- Drops the existing function to replace it with an updated version.
-- Alternatively, CREATE OR REPLACE FUNCTION can be used if preferred,
-- but explicit DROP ensures a clean state if signatures were to change (not the case here).
-- Using CREATE OR REPLACE FUNCTION for simplicity as signature is same.
...
IS 'Handles receiving items for a PO: creates or updates an inventory item batch, updates PO item received quantity, and logs the transaction. p_received_by_user_id is the ID of the user performing the action.';
...
-- The update_purchase_order_status function and its trigger from the original migration
-- (20250506153700_create_po_receiving_logic.sql) are assumed to be in place and correct.
-- This modified handle_receive_po_item_batch function now correctly updates
-- purchase_order_items.quantity_received, which will ensure the trigger fires as intended.
